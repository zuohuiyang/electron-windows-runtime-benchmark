...
早期 SSD 热启动批次复用 profile 并要求 presentedFrames=1，后续还有因账户切换而中断的批次，均不进入最终统计。最终批次重新准备全部样本，采用本文统一方法；仓库仅保留最终批次的原始记录。
...
当前数据足以复算和审查；单主机 20 次／条件、不同 seed 条件、缓存状态与缺少加载轨迹限制了归因和外推。测量结果不代表所有应用或设备都有相同收益。
