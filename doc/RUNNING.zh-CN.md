...
每轮对所有路径各采一次，下一轮反转顺序；20 轮中各有 10 轮正序和逆序。默认 `-Order WarmFirst`：先完成热身和热启动，再逐次重启采冷启动。指定 `-Order ColdFirst`：先逐次重启采冷启动，再在最后一次开机中完成热身和热启动，不增加重启次数。报告记录顺序，并按磁盘、版本、冷热条件和终点分组，不混合计算分位数。
