...
复制完成后只从部署副本启动，两版在每块盘上分别使用独立目录，避免测到构建目录里的文件。复制和预检会读热文件，因此每个冷样本前都要重启；文件哈希在测量之后校验。新目录减少了复用旧测试路径的影响，但不能消除 Windows 缓存、自动预取、杀软和其他后台活动的影响。
...
每轮对所有路径各采一次，下一轮反转顺序；20 轮中各有 10 轮正序和逆序。默认冷启动优先，无需指定 `-Order`：先逐次重启采冷启动，再在最后一次开机中完成热身和热启动。需要先热后冷时，可指定 `-Order WarmFirst`。只比较热启动时，指定 `-WarmOnly`，不安排冷启动、不设置登录续跑，也不重启机器。报告记录顺序，并按磁盘、版本、冷热条件和终点分组，不混合计算分位数。

冷启动优先可以观察没有经过整批热启动训练时的表现，但准备阶段仍会对每个部署路径做一次真实启动预检，因此不是从未运行过的首次启动。脚本保留 Windows 的预取历史，不清理系统缓存。分析学习过程应保留逐轮数据，分别检查初期和后期的变化；仅凭耗时下降不能确认系统预取生效，还需要对应进程的预取事件证据。比较顺序时，两批应采用相同预检、产物和启动方式，历史数据只作为参考。

每次测量前记录控制器 CPU、I/O 和内存优先级；非 Normal / 2 / 5 时停止，避免低优先级改变系统预取条件。这是父进程条件检查，不替代 Electron 进程自身的跟踪证据。
...
- 视频素材随仓库提供，准备时校验 SHA-256；准备和正式测量均不需要下载视频。
...
新实验在父进程调用 `spawn()` 紧前记录 `process.hrtime.bigint()`，用已校验的 QPC 时间轴计算至 APP READY 和视频呈现回调的耗时。这样会计入 `spawn()` 内同步创建进程的工作，而不是从子进程的创建时间戳才开始计时；它也不等同于精确的 `CreateProcessW` API 入口。

配置、原始样本和汇总标记 `timingOrigin=parent-before-spawn-qpc-v1`。原始事件仍保留进程创建时间口径，汇总使用新起点；新实验缺失或混入其他口径时会报错。历史配置没有这一标记，复算时仍按原来的进程创建时间处理，不改变已归档结果。

## 准备和采样流程

1. 在新输出目录复制测量程序，编译原生计时辅助程序，使用仓库视频并校验哈希。
2. 记录机器环境及被测路径，每个部署路径实际启动 Electron 一次，验证启动和视频回调并初始化配置。每次正式测量使用该路径初始配置的独立副本，复制失败时停止准备，不再启动副本进行第二次预检。
3. 保存所有部署目录、测量程序和控制器的文件哈希，设置登录续跑，按 `-Order` 指定的顺序采样。热身与热启动始终在同一次系统启动中完成。
...
# 仅准备，每个部署路径执行一次启动预检；不安装登录启动项、不正式采样、不重启
...
新采集文件保存在独立输出目录，不修改 `data/data/`。使用输出目录里的 `runner/data.cjs report` 复算新实验；`code/scripts/analyze.cjs` 默认复算仓库中的最终批次。新实验独立初始化配置、冻结构建和顺序，不与历史样本合并。
