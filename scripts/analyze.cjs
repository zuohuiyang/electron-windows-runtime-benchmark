...
function verifyArchive() {
  for (const file of read('provenance/SOURCE-FILES.json').files) {
...
function analyze() {
  verifyArchive();
  const cold = read('data/cold/config.json').samples;
  const warm = read('data/warm/config.json').actions;
  const coldState = read('data/cold/state.json'), warmState = read('data/warm/state.json');
  ...
  for (const [items, temperature] of [[cold, 'cold'], [warm, 'warm']]) {
    ...
    for (const a of items) {
      const base = `data/${temperature}/results/${a.id}`;
  ...
  assert.deepEqual(stats, read('data/recorded-summary.json').stats, 'Recomputed values differ from the run-completion report');
