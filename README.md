...
阅读 [方法说明](docs/METHODOLOGY.zh-CN.md) 和 [PR 中文草稿](docs/PR-DRAFT.zh-CN.md)。本仓库中的草稿是后续协作的主版本。
...
运行时、浏览器配置目录、登录凭据和自动登录工具不随仓库分发。视频按固定 Chromium 版本自行准备，见 [MEDIA.md](harness/MEDIA.md)；媒体本身不在此仓库。仓库提供单次测量入口，不包含批量采样控制器或跨机器自动安装器。
