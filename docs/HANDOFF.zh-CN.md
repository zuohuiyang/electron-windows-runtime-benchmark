...
2. 如继续优化，可定位 **APP READY 前约 20 ms 的热启动额外开销**；用户认为主要是预读，本轮仅确认代码路径并披露归因边界，没有要求先追加完整实验。可以在独立诊断分支对加载器入口、预读、LoadLibraryExW、ElectronMain 和 ready 分段计时；诊断产物、样本与当前正式 benchmark 分开。现有证据不足以区分拆分与预读各自的贡献。
