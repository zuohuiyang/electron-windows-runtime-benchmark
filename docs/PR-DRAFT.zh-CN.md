...
我们一个拥有上千万用户的 C 端产品应用本方案后，2024 年 8 月的线上实验数据显示，启动时长 P50 从 6006 ms 降至 5603 ms，减少 403 ms（−6.7%，统计未区分冷、热启动）。
...
| 线上指标 | 改动前 | 改动后 | 变化 |
| --- | ---: | ---: | ---: |
| 首刷时长 P50（不区分冷、热启动） | 6006 ms | 5603 ms | −403 ms（−6.7%） |
...
进程合并不属于本 PR；这组数据反映文件拆分与进程合并的共同效果，不能单独归因于 EXE 拆分。本 PR 的“EXE 拆分＋DLL 预读”整体效果由前述本机 benchmark 对照测量。

预读组总体首刷 P50 已按提交者提供的最新结果更新为 6006 → 5603 ms，绝对变化与相对变化据此计算；其余指标保留团队此前提供的线上统计截图数据，相对变化保留一位小数。主窗口打开耗时未标注分位数。线上“首刷”及冷启动的统计口径独立于本地 benchmark，不与本地视频呈现回调数据合并统计。
