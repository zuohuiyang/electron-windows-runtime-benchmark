...
// Node hrtime uses the QPC axis validated by app/clock.cjs. Include the
// synchronous spawn/CreateProcess work, before the child creation timestamp.
const parentBeforeSpawnNs = process.hrtime.bigint();
const child = spawn(runtime, [path.join(__dirname, 'app')], { env, windowsHide: false, stdio: ['ignore', 'pipe', 'pipe'] });
const parentAfterSpawnNs = process.hrtime.bigint();
const events = [];
...
child.once('close', code => {
  ...
  const unexpectedStderr = unexpected(stderr);
  ...
  const ready = events.find(event => event.event === 'app-ready');
  const frame = frames[0];
  const parentToCreationMs = frame?.creationQpcMs - Number(parentBeforeSpawnNs) / 1e6;
  const parentAppReadyMs = parentToCreationMs + ready?.sinceCreationMs;
  const parentVideoMs = parentToCreationMs + frame?.firstFrameMs;
  const spawnCallMs = Number(parentAfterSpawnNs - parentBeforeSpawnNs) / 1e6;
  ...
  const valid = Boolean(code === 0 && !error && !timedOut && !unexpectedStderr && frames.length === 1 && diagnostics &&
    diagnostics.networkRequests.length === 0 && frame.pid === child.pid && frame.creationEpochMs >= parentSpawnEpochMs - 2 &&
    frame.ipcAndClockDeltaMs >= -2 && frame.ipcAndClockDeltaMs < 500 && ready &&
    Number.isFinite(parentToCreationMs) && parentToCreationMs >= 0 && parentToCreationMs <= spawnCallMs + 2 &&
    Number.isFinite(parentAppReadyMs) && parentAppReadyMs > 0 && Number.isFinite(parentVideoMs) && parentVideoMs > parentAppReadyMs);
  const row = { variant, pid: child.pid, parentSpawnEpochMs, timingOrigin: 'parent-before-spawn-qpc-v1',
    parentBeforeSpawnNs: parentBeforeSpawnNs.toString(), parentAfterSpawnNs: parentAfterSpawnNs.toString(),
    spawnCallMs, parentToCreationMs, parentAppReadyMs, parentVideoMs,
    timestamp: new Date().toISOString(), valid, code, error, timedOut, events, stderr, unexpectedStderr };
  fs.writeFileSync(output, JSON.stringify(row, null, 2), { flag: 'wx' });
  console.log(JSON.stringify({ variant, valid, parentAppReadyMs, parentVideoMs }));
