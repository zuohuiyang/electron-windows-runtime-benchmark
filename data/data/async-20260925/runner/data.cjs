...
function plan(cfg, root) {
  ...
  const phases=cfg.warmOnly===true?['warmup','warm']:(order==='ColdFirst'?['cold','warmup','warm']:['warmup','warm','cold']);
...
function validateRun(root) {
  ...
  for (const item of cfg.samples) {
    ...
    assert.equal(raw.unexpectedStderr ?? raw.stderr, '', '标准错误输出不为空 / Nonempty stderr');
    ...
    rows.push(validate(raw, ctx, read(prefix + '-result.json'), item,
      item.phase === 'cold' ? coldBoots : new Set(), {workflowOnly:cfg.workflowOnly===true,timingOrigin:cfg.timingOrigin}));
  }
...
function report(root) {
  ...
  for(const target of targets) for (const phase of (cfg.warmOnly===true?['warm']:['warm','cold'])) for (const endpoint of ['appReadyMs', 'firstVideoCallbackMs']) {
  ...
  const timingOrigin=cfg.timingOrigin ?? 'process-creation-v1';
  write(path.join(root, 'summary.json'), { timingOrigin,order:cfg.order || 'WarmFirst',warmOnly:!!cfg.warmOnly,workflowOnly:!!cfg.workflowOnly,electron: cfg.electron, targets,user: cfg.user, sid: cfg.sid, stats,comparisons, rows });
  fs.writeFileSync(path.join(root, 'RESULTS.md'), '# Electron 启动测量结果 / Startup benchmark results\n\n' +
    (timingOrigin==='parent-before-spawn-qpc-v1'?'计时起点：父进程 spawn() 前的高精度时间 / Origin: parent QPC immediately before spawn().\n\n':'计时起点：进程创建时间（历史口径） / Origin: process creation time (legacy).\n\n')+
