...
function withParentOrigin(raw) {
 const frame=raw.events.find(e=>e.event==='first-video-frame');
 const ready=raw.events.find(e=>e.event==='app-ready');
 const before=BigInt(Math.round((frame.creationQpcMs-4)*1e6));
 const after=before+6000000n;
 const parentToCreationMs=frame.creationQpcMs-Number(before)/1e6;
 return {...raw,timingOrigin:'parent-before-spawn-qpc-v1',parentBeforeSpawnNs:before.toString(),parentAfterSpawnNs:after.toString(),
  spawnCallMs:6,parentToCreationMs,parentAppReadyMs:ready.sinceCreationMs+parentToCreationMs,parentVideoMs:frame.firstFrameMs+parentToCreationMs};
}
...
test('warm-only plan balances variants and never schedules a reboot',()=>{
 const cfg={count:20,warmups:5,warmOnly:true,targets:['A','C'].map(variant=>({id:variant,variant,storage:'SSD',runtime:variant+'/electron.exe'}))};
 const samples=plan(cfg,'C:/test');
 assert.equal(samples.length,50);
 assert.deepEqual([...new Set(samples.map(s=>s.phase))],['warmup','warm']);
 for(const phase of ['warmup','warm']) {
  const rounds=samples.filter(s=>s.phase===phase);
  assert.deepEqual(rounds.filter(s=>s.round===1).map(s=>s.target),['A','C']);
  assert.deepEqual(rounds.filter(s=>s.round===2).map(s=>s.target),['C','A']);
 }
});
test('warm-only report contains only measured warm-start results',t=>{
 const root=fs.mkdtempSync(path.join(os.tmpdir(),'electron-warm-only-test-'));
 t.after(()=>fs.rmSync(root,{recursive:true,force:true}));
 fs.mkdirSync(path.join(root,'results'));
 const cfg={version:2,warmOnly:true,count:2,warmups:1,sid:'test-sid',user:'test',electron:'target.exe',targets:['A','C'].map(variant=>({id:variant,storage:'SSD',variant,runtime:variant+'/electron.exe'}))};
 ...
 for(const item of cfg.samples){
  ...
  const context={user:cfg.user,sid:cfg.sid,session:1,elevated:false,sample:item,idle:[{cpu:0},{cpu:0},{cpu:0}],boot:new Date(raw.parentSpawnEpochMs-600000).toISOString()};
 ...
 assert.equal(summary.warmOnly,true);
 assert.equal(summary.stats.length,4);
 assert(summary.stats.every(s=>s.phase==='warm'&&s.n===2));
 assert.equal(summary.comparisons.length,2);
 assert.match(fs.readFileSync(path.join(root,'RESULTS.md'),'utf8'),/Warm startup only; no reboot/);
});
...
for(const timingOrigin of [undefined,'parent-before-spawn-qpc-v1'])
for(const variants of [['A'],['A','C']]) test(`two disks, ${variants.length} versions, ${timingOrigin||'legacy'}: complete balanced schedule and grouped results`,t=>{
 ...
 const cfg={version:2,timingOrigin,workflowOnly:variants.length===2,count:20,warmups:5,sid:'test-sid',user:'test',electron:'target.exe',targets:[]};
 ...
 const archived=read(path.join(source,'results/cold-001-ssd-A-sample.json'));
 const original=timingOrigin?withParentOrigin(archived):archived;
 let boot=0;
 for(const item of cfg.samples) {
  const raw=structuredClone(original);raw.variant=item.variant;
 ...
 assert.equal(summary.timingOrigin,timingOrigin||'process-creation-v1');
 if(timingOrigin) {
  for(const stat of summary.stats)assert.equal(stat.p50,original[stat.endpoint==='appReadyMs'?'parentAppReadyMs':'parentVideoMs']);
  assert.match(fs.readFileSync(path.join(root,'RESULTS.md'),'utf8'),/parent QPC immediately before spawn/);
  assert(summary.rows.every(r=>r.appReadyMs>r.creationAppReadyMs));
 }
...
test('parent-origin runs reject missing, inconsistent and mixed timing without falling back to creation time',t=>{
 const {root,cfg,samples}=experiment(t);
 cfg.timingOrigin='parent-before-spawn-qpc-v1';write(path.join(root,'config.json'),cfg);
 assert.throws(()=>validateRun(root),/timing origin mismatch/);
 for(const item of samples) {
  const file=path.join(root,'results',item.id+'-sample.json');
  write(file,withParentOrigin(read(file)));
 }
 assert.equal(validateRun(root).length,4);
 const file=path.join(root,'results',samples[0].id+'-sample.json'),valid=read(file);
 for(const mutation of [
  {parentAppReadyMs:undefined},
  {parentAppReadyMs:valid.parentAppReadyMs+1},
  {parentVideoMs:valid.parentVideoMs-1},
  {parentBeforeSpawnNs:'0'},
  {spawnCallMs:valid.spawnCallMs+1},
  {timingOrigin:'process-creation-v1'}
 ]) {
  write(file,{...valid,...mutation});assert.throws(()=>validateRun(root));
 }
 write(file,valid);assert.equal(validateRun(root).length,4);
 cfg.timingOrigin='invalid';write(path.join(root,'config.json'),cfg);
 assert.throws(()=>validateRun(root),/Unknown timing origin/);
});
